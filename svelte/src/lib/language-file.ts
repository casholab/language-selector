import type { LanguageLookupResult, DisplayOptions, StaticLanguageFile, LoadOptions } from './types.js';
import {loadLanguageData } from './loader.js';
const version = "1.0.4";
...
function validateLanguageData(obj: unknown): LanguageLookupResult {
	...
	for (const entry of Object.values(dataEntries)) {
		if (entry && typeof entry === 'object') {
			...
			if (langData) {
				const hasCode = typeof langData.code === 'string' && langData.code.length > 0;
				const hasName = typeof langData.name === 'string' && langData.name.length > 0;
				const hasEndonym = typeof langData.endonym === 'string' && langData.endonym.length > 0;
	...
	if (!hasValidLanguage) {
		throw new Error('Invalid language data: requires at least 1 language with code and name/endonym');
	}
...
export async function generateStaticDataFile(
    ...
    hydrateFlags: boolean
): Promise<StaticLanguageFile> {
	const languageData = await loadLanguageData(languages, displayOptions, loadOptions)
